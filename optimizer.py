...
class Optimizer:
    ...
    def __init__(
        ...
        terminal_cost: Callable[[jax.Array], jax.Array],
        running_cost: Callable[[Control], jax.Array],
        terminal_cost_grad: Callable[[jax.Array], jax.Array],
        running_cost_grad: Callable[[jax.Array], jax.Array],
        weight: float,
        *,
        u_min: float,
        u_max: float,
    ...
    def total_cost(self, f_traj: Trajectory, control: Control) -> float:
        ...
        return self.terminal_cost(f_traj.get_last_state()) + self.running_cost(control)
    ...
    def _loss_and_grad_adj(self, u_vec: jnp.ndarray) -> Tuple[float, jnp.ndarray]:
        ...
        grad_u = self.running_cost_grad(control) + jnp.sum(rhs * phi_traj.f * self.dx, axis=1)
    ...
    def PMP_check_(self, control_vec, f_traj, phi_traj) -> Tuple[jnp.ndarray, float]:
        ...
        def _pmp_control(f, phi):
            ...
            def _quadratic():
                denom = jnp.maximum(weight, 1e-12)
                return jnp.clip(1-S / denom, u_min, u_max)
            return lax.cond(weight <= 1e-12, _bang_bang, _quadratic)
    ...
    def H(self, f: jax.Array, phi: jax.Array, u: jax.Array) -> Tuple[jnp.ndarray, float]:
        return 0.5 * self.weight * (u - 1.0) ** 2 +  u * jnp.sum(self.solver._coag(f, 1.0) * phi * self.dx)